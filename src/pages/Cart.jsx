// import React, { useContext, useEffect, useState } from 'react'
// import myContext from '../context/data/myContext';
// // import Layout from '../components/Layout';
// import Modal from '../components/Annapurna/Modal';
// import { useDispatch, useSelector } from 'react-redux';
// import { deleteFromCart } from '../components/Annapurna/cartSlice';
// import { toast } from 'react-toastify';
// import { addDoc, collection } from 'firebase/firestore';
// import ReceiptModal from '../components/Annapurna/modal/ReceiptModal';
// import { db } from '../firebase';

// function Cart() {

//   const context = useContext(myContext)
//   const { mode } = context;

//   const dispatch = useDispatch()
//   const cartItems = useSelector((state) => state.cart)
//   // console.log(cartItems)

//   const [totalAmount, setTotalAmount] = useState(0);
//   useEffect(() => {
//     let temp = 0;
//     cartItems.forEach((cartItem) => {
//       temp = temp + parseInt(cartItem.price)
//     })
//     setTotalAmount(temp);
//     // console.log(temp)
//   }, [cartItems])

//   // add to cart
//   const deleteCart = (item) => {
//     dispatch(deleteFromCart(item))
//     toast.success('Deleted from cart');
//   }

//   useEffect(() => {
//     localStorage.setItem('cart', JSON.stringify(cartItems));
//   }, [cartItems])

//   const [name, setName] = useState("")
//   const [phoneNumber, setPhoneNumber] = useState("")

//   const buyNow = async () => {
//     // validation
//     if (name === "" || phoneNumber == "") {
//       return toast.error("All fields are required", {
//         position: "top-center",
//         autoClose: 1000,
//         hideProgressBar: false,
//         closeOnClick: true,
//         pauseOnHover: true,
//         draggable: true,
//         progress: undefined,
//         theme: "colored",
//       })
//     }
//     const addressInfo = {
//       name,
//       phoneNumber,
//       date: new Date().toLocaleString(
//         "en-US",
//         {
//           month: "short",
//           day: "2-digit",
//           year: "numeric",
//         }
//       )
//     }
//     console.log(addressInfo)

//     var options = {
//       key: "",
//       key_secret: "",
//       // amount: parseInt(grandTotal * 100),
//       currency: "INR",
//       order_receipt: 'order_rcptid_' + name,
//       name: "Annapurna",
//       description: "for testing purpose",
//       handler: function (response) {
//         // console.log(response)
//         toast.success('Payment Successful')
//         const paymentId = response.razorpay_payment_id
//         // store in firebase
//         const orderInfo = {
//           cartItems,
//           addressInfo,
//           date: new Date().toLocaleString(
//             "en-US",
//             {
//               month: "short",
//               day: "2-digit",
//               year: "numeric",
//             }
//           ),
//           email: JSON.parse(localStorage.getItem("users")).user.email,
//           userid: JSON.parse(localStorage.getItem("users")).user.uid,
//           paymentId
//         }

//         try {
//           const result = addDoc(collection(db, "orders"), orderInfo)
//         } catch (error) {
//           console.log(error)
//         }
//       },

//       theme: {
//         color: "#3399cc"
//       }

//     };
//     var pay = new window.Razorpay(options);
//     pay.open();
//     console.log(pay)
//   }

//   const [showReceiptModal, setShowReceiptModal] = useState(false);

//   const handlePayOnCounter = () => {
//     setShowReceiptModal(true);
//   };

//   return (
//     // <Layout >
//       <div className="h-screen bg-gray-100 pt-5 " style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
//         <h1 className="mb-10 text-center text-2xl font-bold">Cart Items</h1>
//         <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
//           <div className="rounded-lg md:w-2/3 ">

//             {cartItems.map((item, index) => {
//               return (
//                 <div className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  sm:flex  sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
//                   <img src={item.imageUrl} alt="product-image" className="w-full rounded-lg sm:w-40" />
//                   <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
//                     <div className="mt-5 sm:mt-0">
//                       <h2 className="text-lg font-bold text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{item.title}</h2>
//                       <h2 className="text-sm  text-gray-900" style={{ color: mode === 'dark' ? 'white' : '' }}>{item.description}</h2>
//                       <p className="mt-1 text-xs font-semibold text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{item.price}</p>
//                     </div>
//                     <div onClick={() => deleteCart(item)} className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
//                       <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
//                         <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
//                       </svg>

//                     </div>
//                   </div>
//                 </div>
//               )
//             })}

//           </div>

//           <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
//             <hr className="my-4" />
//             <div className="flex justify-between mb-3">
//               <p className="text-lg font-bold" style={{ color: mode === 'dark' ? 'white' : '' }}>Total</p>
//               <div className>
//                 <p className="mb-1 text-lg font-bold" style={{ color: mode === 'dark' ? 'white' : '' }}>₹{totalAmount}</p>
//               </div>

//             </div>

//             <div className=" flex justify-center ">
//               <button onClick={handlePayOnCounter} type="button" className="focus:outline-none text-white bg-[#001d3d] hover:bg-[#001d3d] focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full  py-2">Pay On Counter
//               </button>

//             </div>
//             {showReceiptModal && <ReceiptModal cartItems={cartItems} totalAmount={totalAmount} closeModal={() => setShowReceiptModal(false)} />}
//             <div className="flex justify-center mb-3">
//               <p className="text-lg font-bold" style={{ color: mode === 'dark' ? 'white' : '' }}>OR</p>

//             </div>
//             <Modal
//               name={name}
//               phoneNumber={phoneNumber}
//               setName={setName}
//               setPhoneNumber={setPhoneNumber}
//               buyNow={buyNow} />

//           </div>
//         </div>
//       </div>

//     // </Layout>
//   )
// }

// export default Cart

import React, { useContext, useEffect, useState } from "react";
import myContext from "../context/data/myContext";
import Modal from "../components/Annapurna/Modal";
import { useDispatch, useSelector } from "react-redux";
import { deleteFromCart, clearCart } from "../components/Annapurna/cartSlice";
import { toast } from "react-toastify";
import { addDoc, collection } from "firebase/firestore";
import { db } from "../firebase";
import ReceiptModal from "../components/Annapurna/modal/ReceiptModal";

function Cart({ user }) {

  const context = useContext(myContext);

  const dispatch = useDispatch();
  const cartItems = useSelector((state) => state.cart);

  const [totalAmount, setTotalAmount] = useState(0);
  useEffect(() => {
    let temp = 0;
    cartItems.forEach((cartItem) => {
      temp = temp + parseInt(cartItem.price);
    });
    setTotalAmount(temp);
  }, [cartItems]);

  // add to cart
  const deleteCart = (item) => {
    dispatch(deleteFromCart(item));
    toast.success("Deleted from cart");
  };

  useEffect(() => {
    localStorage.setItem("cart", JSON.stringify(cartItems));
  }, [cartItems]);

  const [name, setName] = useState("");
  const [phoneNumber, setPhoneNumber] = useState("");

  const buyNow = async () => {
    // validation
    if (name === "" || phoneNumber == "") {
      return toast.error("All fields are required", {
        position: "top-center",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
        pauseOnHover: true,
        draggable: true,
        progress: undefined,
        theme: "colored",
      });
    }
    const addressInfo = {
      name,
      phoneNumber,
      date: new Date().toLocaleString("en-US", {
        month: "short",
        day: "2-digit",
        year: "numeric",
      }),
    };

    var options = {
      key: "",
      key_secret: "",
      currency: "INR",
      order_receipt: "order_rcptid_" + name,
      name: "Annapurna",
      description: "for testing purpose",
      handler: function (response) {
        toast.success("Payment Successful");
        const paymentId = response.razorpay_payment_id;
        const orderInfo = {
          cartItems,
          addressInfo,
          date: new Date().toLocaleString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
          }),
          email: JSON.parse(localStorage.getItem("users")).user.email,
          userid: JSON.parse(localStorage.getItem("users")).user.uid,
          paymentId,
        };

        try {
          const result = addDoc(collection(db, "orders"), orderInfo);
        } catch (error) {
          console.log(error);
        }
      },

      theme: {
        color: "#3399cc",
      },
    };
    var pay = new window.Razorpay(options);
    pay.open();
    console.log(pay);
  };

  const [showReceiptModal, setShowReceiptModal] = useState(false);
  const [isReceiptClosed, setIsReceiptClosed] = useState(true);

//   const handlePayOnCounter = async () => {
//     try {
//       setShowReceiptModal(true);
//       setIsReceiptClosed(false);
      
//       // const userData = JSON.parse(localStorage.getItem("users"));
//       // console.log(userData);

//       const userData = JSON.parse(localStorage.getItem("users"));
//       console.log("userData:", userData);

// // Try accessing the email directly without optional chaining
// const email =  userData.user;
// console.log("Email:", email);


//       // const orderInfo = {
//       //   cartItems,
//       //   totalAmount,
//       //   date: new Date().toLocaleString("en-US", {
//       //     month: "short",
//       //     day: "2-digit",
//       //     year: "numeric",
//       //   }),
//       //   email: userData?.user?.email,
//       //   userid: userData?.user?.uid,
//       // };

//       // const email = userData?.user?.email;
//       // console.log(email);
//       const orderInfo={};
//       if (email) {
//          orderInfo = {
//           cartItems,
//           totalAmount,
//           date: new Date().toLocaleString("en-US", {
//             month: "short",
//             day: "2-digit",
//             year: "numeric",
//           }),
//           email: email,
//           userid: userData?.user?.uid,
//         };
//       }
//       await addDoc(collection(db, "orders"), orderInfo);
//     } catch (error) {
//       console.error("Error placing order:", error);
//     }
//   };
const Useremail=user?.email;
const Userid=user?.uid;
const handlePayOnCounter = async () => {
      try {
        setShowReceiptModal(true);
        setIsReceiptClosed(false);
        const orderInfo = {
          cartItems,
          totalAmount,
          date: new Date().toLocaleString("en-US", {
            month: "short",
            day: "2-digit",
            year: "numeric",
          }),
          email: Useremail,
          userid: Userid,
        };

        await addDoc(collection(db, "orders"), orderInfo);
      } catch (error) {
        console.error("Error placing order:", error);
      }
    };
  const handleReceiptClose = () => {
    //setShowReceiptModal(false);
    setIsReceiptClosed(true);
    dispatch(clearCart()); // Clear cart in Redux state
    localStorage.removeItem("cart"); // Clear local storage
  };

  return (
    <div
      className="h-screen bg-gray-100 pt-5 "
      style={{
        backgroundColor: "dark",
        color: "dark",
      }}
    >
      <h1 className="mb-10 text-center text-2xl font-bold">Cart Items</h1>
      <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
        <div className="rounded-lg md:w-2/3 ">
          {cartItems.map((item, index) => {
            return (
              <div
                className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  sm:flex  sm:justify-start"
                style={{
                  backgroundColor: "dark",
                  color: "dark",
                }}
              >
                <img
                  src={item.imageUrl}
                  alt="product-image"
                  className="w-full rounded-lg sm:w-40"
                />
                <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                  <div className="mt-5 sm:mt-0">
                    <h2
                      className="text-lg font-bold text-gray-900"
                      style={{ color: "dark" }}
                    >
                      {item.title}
                    </h2>
                    <h2
                      className="text-sm  text-gray-900"
                      style={{ color: "dark" }}
                    >
                      {item.description}
                    </h2>
                    <p
                      className="mt-1 text-xs font-semibold text-gray-700"
                      style={{ color: "dark" }}
                    >
                      ₹{item.price}
                    </p>
                  </div>
                  <div
                    onClick={() => deleteCart(item)}
                    className="mt-4 flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      strokeWidth={1.5}
                      stroke="currentColor"
                      className="w-6 h-6"
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
                      />
                    </svg>
                  </div>
                </div>
              </div>
            );
          })}
        </div>

        <div
          className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3"
          style={{
            backgroundColor: "dark",
            color: "dark",
          }}
        >
          <hr className="my-4" />
          <div className="flex justify-between mb-3">
            <p className="text-lg font-bold" style={{ color: "dark" }}>
              Total
            </p>
            <div className>
              <p className="mb-1 text-lg font-bold" style={{ color: "dark" }}>
                ₹{totalAmount}
              </p>
            </div>
          </div>

          <div className=" flex justify-center ">
            <button
              onClick={handlePayOnCounter}
              type="button"
              className="focus:outline-none text-white bg-[#001d3d] hover:bg-[#001d3d] focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm w-full  py-2"
            >
              Pay On Counter
            </button>
          </div>
          {!isReceiptClosed && (
            <ReceiptModal
              isOpen={showReceiptModal}
              cartItems={cartItems}
              totalAmount={totalAmount}
              closeModal={handleReceiptClose}
            />
          )}

          <div className="flex justify-center mb-3">
            <p className="text-lg font-bold" style={{ color: "dark" }}>
              OR
            </p>
          </div>
          <Modal
            name={name}
            phoneNumber={phoneNumber}
            setName={setName}
            setPhoneNumber={setPhoneNumber}
            buyNow={buyNow}
          />
        </div>
      </div>
    </div>
  );
}

export default Cart;
